...
class plate_reader:
  ...
  def read_plate(self,cv_image):
    ...
    img_red1 = self.process_hsv_range(cv_image, redmin1, redmax1)
    img_red2 = self.process_hsv_range(cv_image, redmin2, redmax2)
    img_red = img_red1 + img_red2

    myimg = cv2.cvtColor(cv_image,cv2.COLOR_BGR2LAB)
    myimg_red = cv2.inRange(myimg,(130,200,185),(145,215,205))
    ...
    cv2.imshow("Red", img_red1)
    cv2.waitKey(3)

    cv2.namedWindow("Red2", cv2.WINDOW_NORMAL)
    cv2.imshow("Red",img_red2)
    cv2.waitKey(3)

    cv2.namedWindow("myRed", cv2.WINDOW_NORMAL)
    cv2.imshow("myRed", myimg_red)
    cv2.waitKey(3)



    cv2.imshow("hmmm",cv2.cvtColor(cv_image,cv2.COLOR_BGR2HSV))
